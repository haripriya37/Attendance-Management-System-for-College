...
//	$a=$abs[$i];
	...
	{	//echo "abs : $absentees[$i]    ";	
		...
		$sql1="UPDATE `daily_attendance_cs2` SET `".$d."`=0 WHERE `s_no`=".$i;
...
for($i=1;$i<=sizeof($absentees);$i++)
{
	if($absentees[$i]==1)
	{
$sql2="SELECT `s_no`,`ht_no`,`student_name`,`batch_section` FROM student_data_cs2 WHERE `s_no`='$i'";
